  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 px-3.5 py-6 gap-6">
      {posts.map((item, index) => (
        <Link key={`post-${index}`} className="group" href={item.link}>
          <div className="h-[220px] lg:h-[292px] rounded-md before:rounded-md relative before:absolute before:top-0 before:left-0 before:w-full before:h-full group-hover:before:bg-default-900 group-hover:before:bg-opacity-30">
            ...
            />
            <span className="absolute bottom-2 left-3 text-base transition-all duration-300  text-primary-foreground opacity-0 invisible group-hover:opacity-100 group-hover:visible  group-hover:bottom-4  ">
              {item.title}
            </span>
          </div>
          <div className="mt-4">
            <p className="text-sm font-light  text-default-700">
              Lorem ipsum dolor sit amet consectetur radicalising elit. A eum
              laudanum event dolor, cumquat option?
            </p>
          </div>
