      ...
      <div className="grid grid-cols-2  lg:grid-cols-3 2xl:grid-cols-6 gap-6">
        {data.map((item, index) => (
          ...
          >
            ...
            <div
              className={`text-3xl font-semibold text-${item.color}  dark:text-primary-foreground mt-2`}>{item.total}</div>
            <div
              className="text-sm font-medium text-default-900 dark:text-primary-foreground  capitalize relative z-10">
              {item.text}
            </div>
          </div>
        ))}
