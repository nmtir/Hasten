...
const CreatePriority = ({ open, onClose, priority, priorityId }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (priority) {
      startTransition(() => {
        updatePriority(priorityId, updatedData)
          .then(() => toast.success("Successfully update"))
          .catch(error => console.log(error));
      });
    } else {
      startTransition(() => {
        createUserPriority(data, user.id)
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent hiddenCloseIcon>
        <DialogHeader className="flex-row justify-between items-center py-0 ">
          <DialogTitle className="text-default-900">Create Priority</DialogTitle>
          <DialogClose asChild>
            <div
              type="button"
              size="icon"
        ...
        <DialogDescription className="py-0 pl-1 -mt-2">
          <form onSubmit={handleSubmit(onSubmit)} className=" space-y-5">
            ...
            <div>
              <Label htmlFor="priorityColor" className="text-default-600 mb-1.5">
                Choose Color
              </Label>
              <Input
                type="color"
